...
class train(object):
    ...
    def __init__(self, model, config, args,
                 data_loader, train_logger=None, save_path=None, additional_args=None, visualizer_map=False, return_epe=False):
    ...
    def get_key_map(self, batch):
        ...
        if(isinstance(batch['map_list'][0], dict)):
            map1 = {}
    ...
    def vis_map_RGB(self, maps, name):
        ...
        if(isinstance(maps, dict)):
            for key, map in maps.items():
                map_img_sum = get_map_img_sum(map)
                cv2.imwrite(os.path.join(save_map_path, name.replace("", ".jpg")+"_"+str(key)+".jpg"), map_img_sum)
        else:
            map_img_sum = get_map_img_sum(maps)
